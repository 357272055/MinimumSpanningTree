...
class EdgeNode{
	public:
		...
		E key;
	public:
		EdgeNode():head(-1), tail(-1), key(0){}
		bool operator > (const EdgeNode<T, E>& R){return key > R.key;}
		bool operator <= (const EdgeNode<T, E>& R){return key <= R.key;}
		template<class A, class B>
		friend ostream& operator << (ostream& os, EdgeNode<A, B>& Ed){
			os << Ed.headValue << "-[" << Ed.key << "]-" << Ed.tailValue;
			return os;
		}
};
...
//kruskal
template<class T, class E>
void Kruskal(GraphLnk<T, E>& G, vector<EdgeNode<T, E> >& MST){
	EdgeNode<T, E> edge;
	int u, v, count;
	int numberOfVertex = G.VertexNum();
	int numberOfEdge = G.EdgeNum();
	MinQueue<EdgeNode<T, E> > minQueue;
	UFSets ufsets(numberOfVertex);
	for(u = 0;u < numberOfVertex;++u)
		for(v = u + 1;v < numberOfVertex;++v){
			E weight = G.getWeight(u, v);
			if(weight != G.INFINITY){
				edge.tail = u;
				edge.head = v;
				edge.headValue = G.getValue(v);
				edge.tailValue = G.getValue(u);
				edge.key = weight;
				minQueue.EnQueue(edge);
			}
		}
	count = 1;
	while(count < numberOfVertex){
		minQueue.DeQueue(edge);
		u = ufsets.Find(edge.tail);
		v = ufsets.Find(edge.head);
		if(u != v){
			ufsets.Union(u, v);
			MST.push_back(edge);
			++count;
